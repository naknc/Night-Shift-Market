...
func _build_interface() -> void:
	...
	eyebrow.text = "COZY NIGHT RETAIL"
	eyebrow.add_theme_font_size_override("font_size", 18)
	eyebrow.add_theme_color_override("font_color", Color(0.98, 0.77, 0.49))
	left_content.add_child(eyebrow)
	...
	description.text = "Step into a warm, low-poly market after dark. Organize the floor, keep shelves stocked and make every interaction feel smooth on mobile."
	description.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	description.add_theme_font_size_override("font_size", 22)
	description.add_theme_color_override("font_color", Color(0.93, 0.86, 0.77))
	left_content.add_child(description)
	...
	info_label.text = "Step 1 ships the application shell, persistent settings, save pipeline and scene-loading architecture that future gameplay systems will plug into."
	info_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	info_label.add_theme_font_size_override("font_size", 18)
	info_label.add_theme_color_override("font_color", Color(0.98, 0.90, 0.81))
	...
	var start_button := _build_action_button("New Shift", Color(0.36, 0.20, 0.10), Color(1.0, 0.95, 0.88))
	start_button.pressed.connect(func() -> void:
		new_game_requested.emit()
	)
	button_column.add_child(start_button)

	_continue_button = _build_action_button("Continue", Color(0.47, 0.28, 0.13), Color(1.0, 0.95, 0.88))
	_continue_button.pressed.connect(func() -> void:
		continue_game_requested.emit()
	)
	button_column.add_child(_continue_button)

	var settings_button := _build_action_button("Settings", Color(0.63, 0.41, 0.23), Color(0.20, 0.11, 0.05))
	settings_button.pressed.connect(func() -> void:
		settings_requested.emit()
	)
	button_column.add_child(settings_button)

	var exit_button := _build_action_button("Exit", Color(0.83, 0.67, 0.53), Color(0.20, 0.11, 0.05))
	exit_button.pressed.connect(func() -> void:
		exit_requested.emit()
	)
	button_column.add_child(exit_button)

	_save_hint_label = Label.new()
	_save_hint_label.text = "Continue unlocks after the first shift is started."
